...
const Discover = () => {
	...
	const handleConnect = (m?: any) => {
		if (m?.id) {
			setRequestedConnections((prev) => new Set(prev).add(m.id));
			toast({
	...
	const renderPersonalityColor = (match: any) => {
		// Use personalityColor if available, otherwise calculate from scores
		if (match.personalityColor) {
			return match.personalityColor;
		}
		try {
			...
			const dominant = (
				Object.keys(dimensionPercentMap) as Array<keyof typeof dimensionPercentMap>
			).reduce((a, b) => (dimensionPercentMap[a] >= dimensionPercentMap[b] ? a : b));
				...
				<div className="flex items-center justify-between animate-slide-up">
					<div>
						...
						<p className="text-muted-foreground">
							Browse friends and financial advisors who match your financial
							personality
						</p>
					</div>
				...
				<div className="bg-white rounded-lg shadow-card divide-y">
					{pageItems.length === 0 ? (
						...
						pageItems.map((m: any, idx: number) => {
							...
							return (
								<div key={m.id ?? idx} className="flex items-center p-4 gap-4">
									...
									<div className="flex-1 min-w-0 ml-2">
										...
										<div className="mt-3 text-sm text-muted-foreground flex flex-wrap gap-2">
											{m.tags?.slice(0, 3).map((t: string) => (
												<span
													key={t}
													className="px-2 py-1 bg-muted/10 rounded-full text-xs"
												>
													{t}
												</span>
											))}
									...
									<div className="ml-4 flex flex-col gap-2">
										...
										<Button
											...
											disabled={requestedConnections.has(m.id)}
										>
											{requestedConnections.has(m.id)
												? "Requested"
												: "Connect"}
										</Button>
									</div>
					...
					<div className="relative bg-white rounded-lg shadow-lg max-w-2xl w-full mx-4 p-6">
						<div className="flex gap-4">
							...
							<div className="flex-1">
								<div className="flex items-start justify-between">
									<div>
										...
										{selectedProfile.role === "advisor" && (
											<span className="inline-block mt-1 px-2 py-1 bg-blue-50 text-blue-600 rounded-full text-sm font-medium">
												Financial Advisor
											</span>
										)}
									</div>
								...
								<div className="mt-4">
									...
									>
										{selectedProfile.personalityDescription ||
									...
									<div className="mt-3 flex flex-wrap gap-2">
										{(selectedProfile.tags || []).map((t: string) => (
											<span
												key={t}
												className="px-3 py-1 bg-muted/10 rounded-full text-sm"
											>
												{t}
											</span>
										))}
									...
									<div className="mt-3 grid grid-cols-2 gap-2 text-sm">
										<div>
											...
											<div className="font-medium">
												{selectedProfile.personalityPercentages?.longTerm ??
													"—"}
												%
											</div>
										</div>
										<div>
											...
											<div className="font-medium">
												{selectedProfile.personalityPercentages?.lowRisk ??
													"—"}
												%
											</div>
										</div>
										<div>
											...
											<div className="font-medium">
												{selectedProfile.personalityPercentages
													?.complexity ?? "—"}
												%
											</div>
										</div>
										<div>
											...
											<div className="font-medium">
												{selectedProfile.personalityPercentages?.lumpSum ??
													"—"}
												%
											</div>
										</div>
						...
						<div className="mt-6 flex justify-end gap-3">
							...
							<Button
								...
								disabled={requestedConnections.has(selectedProfile.id)}
							>
								{requestedConnections.has(selectedProfile.id)
									? "Requested"
									: "Connect"}
							</Button>
						</div>
